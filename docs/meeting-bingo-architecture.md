...
graph TD
    subgraph Browser["BROWSER ENVIRONMENT"]
        subgraph App["REACT APPLICATION"]
            ...
            GL["Game Logic\nCard Gen · Win Check · Scoring"]
            SR["Speech Recognition\nWeb Speech API · Transcript Processing · Word Detection"]
            CTX["Context State"]
...
```mermaid
flowchart TD
    subgraph CG["1. CARD GENERATION FLOW"]
        direction LR
        SC[Select Category] --> SW[Shuffle Words] --> PW[Pick 24 Words] --> RC[Render Card]
    end

    subgraph SRF["2. SPEECH RECOGNITION FLOW"]
        direction LR
        AI[Audio Input] --> SAPI[Speech API] --> TT[Transcript Text] --> WD[Word Detection]
        WD --> MF{Match Found?} --> FS[Fill Square] --> UUI[Update UI]
    end

    subgraph WDF["3. WIN DETECTION FLOW"]
        direction LR
        SqF[Square Filled] --> CL[Check Lines] --> BQ{BINGO?} --> CS[Celebrate & Share]
    end
```
...
```mermaid
graph TD
    START["FUTURE: MULTIPLAYER"]

    START --> A["Option A: WebSocket Server\nRequires Backend\nReal-time sync · Shared game state\nLive leaderboard · Cost: $5–20/month"]
    START --> B["Option B: Peer-to-Peer WebRTC\nNo backend required\nDirect browser connections\nLimited to ~10 players · Cost: Free"]
    START --> C["Option C: Firebase Realtime DB\nManaged real-time sync\nFree tier generous · Easy implementation\nFree up to 100 concurrent"]
```
